...
local cbEvent = '__desync_cb_%s'
local callbackTimeout = GetConvarInt('desync:callbackTimeout', 300000)
...
local function triggerClientCallback(_, event, playerId, cb, ...)
    ...
    TriggerClientEvent('desync-lib:validateCallback', playerId, event, cache.resource, key)
...
function lib.callback.register(name, cb)
    ...
    RegisterNetEvent(event, function(resource, key, ...)
        TriggerClientEvent(cbEvent:format(resource), source, key, callbackResponse(pcall(cb, ...)))
    end)
end
